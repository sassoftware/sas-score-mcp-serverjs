...
def slide1():
    ...
    # Subtitle
    draw.text((px(0.35), px(1.78)), "AI-Powered SAS Viya Integration Workflow",
...
def slide2():
    ...
    layers = [
        ...
        ("Skills Layer",         "Request Routing  ·  Decision Trees  ·  Strategies",                                  BLUE),
        ("MCP Server (Node.js)", "Tool Registration  ·  Two-Tier Descriptions  ·  Session Cache",                      TEAL),
    ...
    for i, (label, sub, bg) in enumerate(layers):
        ...
        text_in_box(draw, sub, 3.1, ly+0.05, 6.4, LH-0.1,
                    font(px(0.10)), WHITE, valign="middle")
        if i < len(layers)-1:
            ax, ay = px(4.85), px(ly+LH)
            draw.rectangle([ax, ay, ax+px(0.3), ay+px(0.065)], fill=TEAL)
        ly += LH + 0.065
...
# ════════════════════════════════════════════════════════════════════════════
# SLIDE 4 — Skills Architecture & Decision Trees
# ════════════════════════════════════════════════════════════════════════════
def slide4():
    img = new_slide(OFFWH)
...
def slide5():
    ...
    steps = [
        ("1","User Request",    '"Score customer with churn model  (age=45, income=60000)"',      NAVY,       False),
        ("2","Skills Layer",    "score-strategy skill routes to mas-score via decision tree",      BLUE,       False),
        ("3","Verify",          "find-mas confirms churnRisk model exists in SAS Viya",            c("2E6DA4"),False),
        ("4","MCP Tool",        "mas-score tool invoked with model name + scenario",               TEAL,       False),
        ("5","Helper / Restaf", "_masScoring() → masSetup() → masDescribe() → masRun()",          TEALD,      True),
        ("6","Result",          '{ prediction:"churn", probability:0.78, age:45, income:60000 }', c("0D4F3A"),True),
    ]
